function MPI_COMM_WORLD = OctMPI_Comm_init(n_proc,machines)
% OctMPI_Comm_init  -  Creates generic communicator.
%
...
%  if (isunix) host = getenv('HOST');          end
  ...
  machine_db.octave_command = cell(1,n_m);	% Octave command.
  ...
  % Set paths.
  [pwd_pc pwd_unix] = OctMPI_dir_map(machine_db,pwd);

  % Set machine_db values.
  for ii=1:n_m
    machine_db.type{1,ii} = default_type;
    machine_db.machine{1,ii} = host;
    machine_db.dir{1,ii} = [pwd '/OctMPI'];
    machine_db.octave_command{1,ii} = machine_db_settings.octave_command;
    ...
    if (n_machines > 0)
      ...
      if (strcmp(type,'pc'))
        machine_db.dir{1,ii} = [pwd_pc '\\OctMPI'];
      endif
      if (strcmp(type,'unix'))
        machine_db.dir{1,ii} = [pwd_unix '/OctMPI'];
      endif
