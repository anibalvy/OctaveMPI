function MPI_Abort()
% MPI_Abort  -  Aborts any currently running OctaveMPI sessions.
%
%   MPI_Abort()
%
%   Will abort any currently running OctaveMPI sessions.
%   by looking for leftover Octave jobs and killing them.
%   Cannot be used after OctMPI_Delete_all.
%
    ...
    for i_file=1:n_files
      ...
      if (dir_sep)
 	   ...
 	   if (strcmp(machine, host))
              % Don't do anything, otherwise MPI_Abort will kill the instance of Octave that launched
              % the MPI_Abort command
              % unix_command = [ 'taskkill /f /FI "USERNAME eq ' username '" /im matlab.exe' q ];
              unix_command = '';
	   else
              % WinXP
              unix_command = [ remote_launch machine remote_flags ' ' q 'taskkill /f /FI "USERNAME eq ' username '" /im matlab.exe' q ];
              ...
              if (ispc)
                 unix_command = [ remote_launch machine remote_flags ' ' qq 'taskkill /f /FI \\"USERNAME eq ' username '\\" /im matlab.exe' qq ];
              endif
           end
