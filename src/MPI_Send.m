...
%
  ...
  buffer_file = OctMPI_Buffer_file(my_rank,dest,tag,comm);
  lock_file = OctMPI_Lock_file(my_rank,dest,tag,comm);

  % Save buf to file.
  save(buffer_file,'varargin');
  %unix(['chmod 777 ' buffer_file]);

  % Create lock file.
  fclose(fopen(lock_file,'w'));
  %unix(['chmod 777 ' lock_file]);
